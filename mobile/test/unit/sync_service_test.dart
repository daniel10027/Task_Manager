...
void main() {
  ...
  test('drains a queued create and reconciles the local id with the server id', () async {
    final localTask = buildTask(localId: 'local-1', id: null, title: 'Buy milk');
    await cache.put(localTask);
    await queue.enqueueCreate('local-1', localTask.toRequestJson());

    final serverTask = buildTask(localId: 'server-1', id: 100, title: 'Buy milk');
    when(() => taskApi.create(
    ...
    expect(cache.get('local-1'), isNull, reason: 'old local-id entry should be gone');
  ...
  test('a network failure stops the drain and keeps the op queued for retry', () async {
    await queue.enqueueCreate('local-1', {'title': 'A', 'description': '', 'status': 'TODO'});
    await queue.enqueueCreate('local-2', {'title': 'B', 'description': '', 'status': 'TODO'});

    when(() => taskApi.create(
          title: any(named: 'title'),
          description: any(named: 'description'),
          status: any(named: 'status'),
        )).thenThrow(DioException(requestOptions: RequestOptions(path: '/api/tasks')));

    final result = await sync.drain();

    expect(result, SyncResult.partialFailure);
    expect(queue.pendingCount, 2, reason: 'nothing should be dropped on network failure');
    expect(queue.all.first.retryCount, 1);
    verify(() => taskApi.create(
          title: any(named: 'title'),
          description: any(named: 'description'),
          status: any(named: 'status'),
        )).called(1);
  ...
  test('a 404 on an update is treated as resolved and the op is dropped', () async {
    await queue.enqueueUpdate('55', {'title': 'x', 'description': '', 'status': 'TODO'});
